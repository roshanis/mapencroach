...
# A real, non-default secret: create_app now fails closed outside demo mode
# if MAPENCROACH_JWT_SECRET is unset or equal to auth.py's dev default, so
# tests need a signing secret that would actually pass that check.
...
def _jwt_secret_env(monkeypatch):
    """Give every test a valid, non-default signing secret so create_app's
    fail-closed startup check passes outside demo mode. Demo-mode tests
    delenv this before creating their app, since MAPENCROACH_DEMO=1 refuses
    to start alongside a custom secret."""
